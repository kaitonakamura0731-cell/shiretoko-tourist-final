...
export const subscriptions: SubscriptionPlan[] = [
  {
    ...
    note: '収穫最盛期の2回をまとめてお届けするプラン案です。変更・スキップは注文前にご相談ください。',
  },
  {
    ...
    discount: '約8%OFF',
    interval: '隔月（年6回）',
    note: '常備野菜を切らさない定番便のプラン案です。お届け間隔の変更・解約は注文前にご相談ください。',
  },
  {
    slug: 'potato-8kg',
    ...
    discount: '5%OFF',
    interval: '3か月ごと（年4回）',
    note: '品種を変えて旬を楽しむ食べ比べ便のプラン案です。1回だけのお試し利用も検討できます。',
  },
];
...
export const pointsInfo: PointsInfo = {
  ...
  note: '購入金額（税込）の1%をポイント目安として表示します。利用条件は注文前にご確認ください。',
};
...
export const thresholdGift: ThresholdGift = {
  ...
  note: 'カートが条件を満たすと案内を表示します。在庫状況によりおまけの品は変わります。',
};
...
export const referral: Referral = {
  ...
  steps: [
    '会員ページで紹介クーポン案を確認',
    '紹介コードや案内文の運用条件を決める',
    '導入する場合は双方の特典条件を注文画面に反映',
  ],
  note: 'これはオプション案です。実際の付与条件・利用条件は運用開始時に確定します。',
};
...
export const birthdayCoupon: BirthdayCoupon = {
  ...
  validDays: 30,
  description:
    '会員情報にお誕生月をご登録いただくと、その月のクーポン案内を受け取れます。自分へのご褒美に旬の一品をどうぞ。',
  note: 'クーポンの配布方法は運用開始時に設定します。¥3,000以上のご注文で利用する案です。',
};
...
export const restockInfo: RestockInfo = {
  channel: 'LINE',
  description:
    '売り切れ商品の再入荷案内をLINEで受け取れるようにするオプションです。人気の旬商品を逃しにくくします。',
  steps: [
    '商品ページの「再入荷案内」ボタンをタップ',
    '公式LINEを友だち追加',
    '入荷時にLINEで案内',
  ],
  targets: [
